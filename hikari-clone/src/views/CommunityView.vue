...
// 1. 模拟数据：顶部的热门版块
const hotSections = [
  { id: 1, title: '闲聊灌水', count: '78图文', posts: '31文章', color: 'from-blue-400 to-blue-300', icon: ChatbubbleOutline },
  { id: 2, title: 'Galgame周报', count: '3图文', posts: '32文章', color: 'from-orange-400 to-orange-300', icon: MegaphoneOutline },
  { id: 3, title: '答疑解惑', count: '37图文', posts: '7文章', color: 'from-green-400 to-green-300', icon: CreateOutline },
  { id: 4, title: '新人报道区', count: '31图文', posts: '5文章', color: 'from-pink-400 to-pink-300', icon: PersonOutline }, 
]
    ...
    <div class="container mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-4 gap-6 relative z-10">
      ...
      <main class="lg:col-span-2 space-y-6">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div v-for="sec in hotSections" :key="sec.id" 
               class="bg-white rounded-lg p-3 shadow-sm border border-gray-100 flex items-center gap-3 cursor-pointer hover:shadow-md transition group">
            <div :class="`w-12 h-12 rounded-lg bg-gradient-to-br ${sec.color} flex items-center justify-center text-white shadow-sm group-hover:scale-110 transition`">
               <n-icon size="24" :component="sec.icon" />
            </div>
            <div>
              <div class="font-bold text-gray-700 text-sm group-hover:text-hikari-blue">{{ sec.title }}</div>
              <div class="text-xs text-gray-400 scale-90 origin-left mt-0.5">{{ sec.count }} · {{ sec.posts }}</div>
            </div>
          </div>
        </div>
      ...
      <aside class="hidden lg:block lg:col-span-1 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 border border-gray-100 sticky top-24">
          ...
          <ul class="space-y-4">
            <li v-for="(topic, index) in trendingTopics" :key="topic.id" class="flex items-start gap-3 cursor-pointer group">
